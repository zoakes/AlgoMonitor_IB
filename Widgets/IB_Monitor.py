        ...
        while True:
            try:
                ...
                for instrument in open_insts:
                    ...
                    if opl < self.low_pnl[instrument]:
                        ...
                        if self.low_pnl[instrument] < self.largest_loss:
                            ...
                            self.log(f'Largest Open Loss -- {self.largest_loss} : {instrument.symbol}')
                            
                        '''Alternate method of tracking wins/losses'''    
                        ...
                        #if self.hi_pnl[instrument] >= PT and instrument not in open_insts:
                            #self.trade_list.append(1)
                            #self.log(f'Win Added -- {instrument.symbol}')
                ...
                for nid in new_ids:
                    ...
                    self.single_PNLs[nid] = single_pnl                          #TRACK THEM GLOBALLY -- to ref later in avg w/l
                    self.open_PNL += single_pnl
                self.log(f'Open PNL -- {self.open_PNL')
                
                #TRACK WINS/LOSSES HERE --
                ...
                #Max DD
                wndw = len(ordered_pnl)
                rolling_max = ordered_pnl.rolling(window=wndw).max()
                rolling_min = ordered_pnl.rolling(window=wndw).min()
                max_dd = (rolling_max - rolling_min)/rolling_max
                if max_dd < self.max_DD:
                    self.max_DD = max_dd
                    self.log(f'Max Drawdown -- {self.max_dd}')
                    
                #Simpler REALIZED largest loss calc    
                largest_loss = min(np.diff(ordered_pnl,prepend=0))
                if largest_loss > self.largest_loss_r:
                ...
                for t in self.trade_list[::-1]:
                    if t != 0:
                        ...
                        break
                self.log(f'Consecutive Wins -- {self.consec_wins -1}')
                ...
                for t in self.trade_list[::-1]:
                    if t != 1:
                        ...
                        break
                    self.log(f'Consecutive Losses -- {self.consec_loss - 1}')
                
                #Average Win / Loss + Average Trade
                #for sid in self.set_ids: -- Easier way to do this.
